...
# === (0) Preprocessing: 
gmx editconf -center 0 0 0 -f ${PDB} -o ${PDB}

# === (1) pdb4amber: 
pdb4amber -i $PDB -o pdb4amber.pdb

# NOTE: 
# The connect lines in the PDB generated by `pdb4amber` seem invalid!
# because the lines indicate CB-CB bonds, which must be SG-SG.
sed -i.bak '/CONECT/d' pdb4amber.pdb #Delete CONECT lines because of that.

# NOTE: 
# Since `pdb4amber_sslink.txt` has correct atom numbers for SG-SG bonds, 
# the numbers are extracted to ssbonds.txt, 
# which shall be used for tleap input file.
cat pdb4amber_sslink | \
    awk '{print "bond mol." $1 ".SG" " "  "mol." $2 ".SG"}' > ssbonds.txt
...
cat templates/tleap.in | \
    sed -e "s!#{INPUT}!${PDB}!g" \
        -e "s!#{FF}!${FF}!g" \
        -e "/#{SSBONDS}/r ssbonds.txt"  > tleap.in
rm ssbonds.txt
...
echo Protein | gmx trjconv -f system.gro -s system.gro -o system.gro -n index_ProeinIon.ndx
